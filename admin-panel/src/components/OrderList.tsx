...
export const OrderList: React.FC = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      const totalCount = meta?.total ?? ordersList.length;
      setTotalOrders(totalCount);

      if (meta?.stats) {
        setOverallStats({
          totalOrders: meta.stats.totalOrders ?? totalCount,
          totalRev: meta.stats.totalRev ?? 0,
          codCount: meta.stats.codCount ?? 0,
          onlineCount: meta.stats.onlineCount ?? 0,
          pendingDispatch: meta.stats.pendingDispatch ?? 0,
        });
      } else {
        const totalRev = ordersList.reduce((acc: number, ord: any) => acc + ((ord.subtotal || 0) + (ord.shippingCost || 0) - (ord.couponDiscount || 0)), 0);
        const codCount = ordersList.filter((o: any) => o.paymentType === 'COD').length;
        const onlineCount = ordersList.filter((o: any) => o.paymentType === 'ONLINE').length;
        const pendingDispatch = ordersList.filter((o: any) => o.status === 'CONFIRMED' || o.status === 'PAID' || o.status === 'INITIALIZED').length;
        setOverallStats({
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      setMessage(`Order #${selectedOrder.id.slice(-6).toUpperCase()} permanently deleted from database.`);
      setShowDeleteConfirmModal(false);
      setShowOrderModal(false);
      setSelectedOrder(null);
      fetchOrders();
    } catch (err: any) {
      setMessage(err.response?.data?.message || err.message || 'Failed to delete order.');
    } finally {
      setIsDeleting(false);
    }
  };
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      await api.patch(`/orders/${selectedOrder.id}`, { status: 'CANCELLED' });
      setMessage(`Order #${selectedOrder.id.slice(-6).toUpperCase()} has been cancelled.`);
      ...
      setMessage(err.response?.data?.message || err.message || 'Failed to cancel order.');
    } finally {
    ...
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3.5 w-full max-w-full min-w-0">
        <div className="bg-white p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 shadow-xs flex items-center gap-2.5 sm:gap-3.5 min-w-0 overflow-hidden">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-sm sm:text-xl font-black text-slate-800 truncate">{overallStats.totalOrders || totalOrders || orders.length}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 shadow-xs flex items-center gap-2.5 sm:gap-3.5 min-w-0 overflow-hidden">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-sm sm:text-xl font-black text-slate-800 truncate">₹{overallStats.totalRev.toLocaleString('en-IN')}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 shadow-xs flex items-center gap-2.5 sm:gap-3.5 min-w-0 overflow-hidden">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-sm sm:text-xl font-black text-slate-800 truncate">{overallStats.pendingDispatch}</h3>
          </div>
        </div>
        ...
        <div className="bg-white p-2.5 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 shadow-xs flex items-center gap-2.5 sm:gap-3.5 min-w-0 overflow-hidden">
          ...
          <div className="min-w-0 flex-1">
            ...
            <h3 className="text-[11px] sm:text-sm font-black text-slate-800 truncate">
              <span className="text-amber-600">{overallStats.codCount} COD</span> / <span className="text-indigo-600">{overallStats.onlineCount} Online</span>
            </h3>
          </div>
        </div>
      </div>
            ...
            <div className="flex items-center justify-between border-b border-slate-100 px-4 py-3 sm:px-6 sm:py-4 bg-white shrink-0 w-full max-w-full overflow-hidden">
              ...
              <div className="flex items-center gap-1.5 shrink-0">
                ...
                <button
                  ...
                  className="p-1.5 hover:bg-rose-50 text-slate-400 hover:text-rose-600 rounded-xl transition cursor-pointer border border-transparent hover:border-rose-100"
            ...
            <div className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-4 sm:p-6 space-y-4 min-w-0 w-full max-w-full">
              ...
              <div className="bg-slate-50 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border border-slate-100 flex flex-col justify-between space-y-3 min-w-0">
                ...
                <div className="pt-2 border-t border-slate-200/60 space-y-2.5">
                  ...
                  {/* Cancel Order Action - Visible only BEFORE SHIPPED */}
                  {['PENDING', 'INITIALIZED', 'CONFIRMED', 'PAID'].includes(selectedOrder.status) && (
                    <div className="pt-1">
                      <button
                        type="button"
                        onClick={() => setShowCancelConfirmModal(true)}
                        disabled={loading}
                        className="w-full bg-white hover:bg-rose-50 text-rose-600 hover:text-rose-700 border border-rose-200/80 font-bold py-2 px-3 rounded-xl text-xs transition flex items-center justify-center gap-1.5 shadow-2xs cursor-pointer active:scale-[0.99] disabled:opacity-50"
                      >
                  ...
                  <div className="pt-2 border-t border-slate-200/60 flex items-center justify-between">
                    <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Danger Zone</span>
                    <button
                      type="button"
                      onClick={() => setShowDeleteConfirmModal(true)}
                      disabled={loading}
                      className="text-[11px] font-extrabold text-rose-600 hover:text-rose-700 hover:bg-rose-50 border border-rose-200/80 px-2.5 py-1.5 rounded-lg flex items-center gap-1.5 transition cursor-pointer shadow-2xs disabled:opacity-50 active:scale-95"
                    >
      ...
      {showDeleteConfirmModal && selectedOrder && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 overflow-y-auto">
          ...
          />
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-5 sm:p-6 z-10 overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200 space-y-4">
            <div className="flex items-start gap-3.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-xs text-slate-500 mt-0.5">
                  Order <span className="font-mono font-bold text-slate-800">#{selectedOrder.id.slice(-6).toUpperCase()}</span>
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="bg-slate-50 border border-slate-200/80 rounded-xl p-3 text-xs space-y-1.5">
              <div className="flex justify-between items-center">
                ...
                <span className="font-extrabold text-slate-800 truncate max-w-[200px]">
                  {selectedOrder.createdBy?.user?.name || selectedOrder.createdBy?.user?.email || 'Customer Profile'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-medium">Current Status:</span>
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-extrabold ${getOrderStatusColor(selectedOrder.status)}`}>
                  {selectedOrder.status}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-slate-500 font-medium">Items Count:</span>
                <span className="font-bold text-slate-800">{selectedOrder.items?.length || 0} line item(s)</span>
              </div>
            </div>

            {/* Warning Notice */}
            <div className="p-3 bg-rose-50 border border-rose-200/80 rounded-xl text-rose-900 text-xs space-y-1.5">
              ...
              <p className="text-[11px] text-rose-600 leading-relaxed font-medium">
                This order will be completely purged from the database along with associated shipments, tracking, and notification records. Any reserved warehouse inventory will be automatically restored. This action cannot be undone.
              </p>
              {(selectedOrder.status === 'SHIPPED' || selectedOrder.status === 'DELIVERED') && (
                <div className="text-[10px] font-bold text-rose-800 bg-rose-100/80 p-2 rounded-lg mt-1 border border-rose-200">
                  ⚠️ Caution: This order is already marked as {selectedOrder.status}. Verify with courier and customer before deleting.
                </div>
              )}
            </div>
            ...
            <div className="flex items-center justify-end gap-2.5 pt-2">
              ...
              <button
                ...
                className="px-4 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-xs font-extrabold transition cursor-pointer flex items-center gap-2 shadow-xs active:scale-95 disabled:opacity-50"
              >
                {isDeleting ? (
                  <>
      ...
      {showCancelConfirmModal && selectedOrder && typeof document !== 'undefined' && createPortal(
        <div className="fixed inset-0 z-60 flex items-center justify-center p-4 overflow-y-auto">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-5 sm:p-6 z-10 overflow-hidden border border-slate-100 animate-in zoom-in-95 duration-200 space-y-4">
            ...
            <div className="p-3 bg-amber-50 border border-amber-200/80 rounded-xl text-amber-900 text-xs space-y-1.5">
              <p className="font-bold flex items-center gap-1.5 text-amber-800">
                Order Status Will Be Marked CANCELLED
              </p>
              <p className="text-[11px] text-amber-700 leading-relaxed font-medium">
                Cancelling this order stops dispatch workflows and automatically restores deducted item quantities back to warehouse inventory stock.
              </p>
            </div>
